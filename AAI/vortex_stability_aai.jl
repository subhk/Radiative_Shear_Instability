...
function meanflow!(mf, Op, params, grid, T)
    ...
    file = jldopen("eddy_structure_nd_72hrs_aai.jld2")
    rn   = file["r"];   zn   = file["z"]
    Un   = file["U"];   Bn   = file["B"]
...
function solve_AAI2d(m, λref, ra, rb) #,λₛ)
    ...
    #     construct_linear_map(𝓛, ℳ), nev=25, tol=0.0, restarts=300, which=LR()
    #     )
...
function EigsSolver()
    ...
    for it ∈ 1:1:20
        ...
        @time r, z, λₛ[it], Χ, N = solve_AAI2d(m[it], λref, ra, rb) #, it, λ)
